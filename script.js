(() => {
  ...
  const DEFAULT_FURNITURE_DATA = [
    ...
    },
    {
      ...
      wholeImage: "images/zhonggu/上下收纳层架/微信图片_20260410165002.jpg"
    },
    {
      category: "vintage",
      subcategory: "柜架类",
  ...
  const toBannerImagePath = (fileName) => {
    const normalized = String(fileName || "").replaceAll("\\", "/").split("/").pop()?.trim() || "";
  ...
  const setupAdminPanel = () => {
    ...
    const debouncedBannerChangeHandler = createDebouncedHandler(async (event) => {
      ...
      setUploadStatus("轮播图路径已加入，请确保图片文件位于 images 目录。", false);
      try {
        for (const file of files) {
          ...
          const path = toBannerImagePath(file.name);
          if (path) bannerImageUrls.push(path);
        }
      } catch {
        alert("图片读取失败，请重试。");
      }
